...
function PersonIcon({ size = 36, avatarUrl }) {
  return (
    <div style={{
...
function ConfirmLogoutModal({ currentUser, onConfirm, onCancel }) {
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 16px' }}>
      <div style={{ background: 'var(--b2)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 20, padding: '32px 28px', width: '100%', maxWidth: 340, boxShadow: '0 30px 80px rgba(0,0,0,0.7)', display: 'flex', flexDirection: 'column', alignItems: 'center', animation: 'modalIn .18s ease' }}>
        ...
        <div style={{ display: 'flex', gap: 10, width: '100%' }}>
          <button onClick={onCancel} style={{ flex: 1, padding: '11px', borderRadius: 11, background: 'var(--b3)', border: '1px solid var(--br)', color: 'var(--t2)', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'Space Grotesk, sans-serif', transition: 'all .2s' }}>Rămân</button>
          <button onClick={onConfirm} style={{ flex: 1, padding: '11px', borderRadius: 11, background: 'linear-gradient(135deg, #7f1d1d, #ef4444)', border: '1px solid rgba(239,68,68,0.4)', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'Space Grotesk, sans-serif', boxShadow: '0 4px 20px rgba(239,68,68,0.3)' }}>↩ Deconectare</button>
        </div>
      </div>
...
export default function Panel({ currentUser: initialUser, onLogout }) {
  ...
  // Închide sidebar la resize dacă e desktop
  useEffect(() => {
    function onResize() {
  ...
  function navigate(id) {
    setView(id);
    if (id !== 'members') setSelMember(null);
    setSidebarOpen(false); // închide sidebar pe mobile după navigare
  }
  ...
  const navItems = [
    ...
    { id: 'activity',      label: 'Activitate PR',     icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg> },
    ...
    { id: 'events',        label: 'Evenimente',        icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> },
    ...
    ...(isAdj ? [{ id: 'logs', label: 'Sistem Logs', section: !isSef ? 'Admin' : undefined, icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg> }] : []),
  ];
  ...
  return (
    <>
      {showProfilepage && <Profilepage currentUser={currentUser} onClose={() => setShowProfilepage(false)} />}
      {viewMemberProfile && (
        <Profilepage
      ...
      <style>{`
        @media (max-width: 768px) {
          .panel-wrap { grid-template-columns: 1fr !important; }
          .sidebar {
          ...
          .two-col, .two-eq { grid-template-columns: 1fr !important; }
          .tb-title { font-size: 14px !important; }
          .tb-sub { display: none; }
          .modal { width: calc(100vw - 20px) !important; margin: 0 10px !important; }
        }
        ...
        {sidebarOpen && (
          <div
            ...
            style={{
              ...
              zIndex: 599, backdropFilter: 'blur(3px)',
            }}
          />
        )}
            ...
            <div className="tb-right">
              ...
              <div ref={profileRef} style={{ position: 'relative' }}>
                <div onClick={() => setProfileOpen(p => !p)}>
                  <PersonIcon size={36} avatarUrl={currentUser.avatarUrl} />
                </div>

                {profileOpen && (
                  <div style={{
                    position: 'absolute', top: 48, right: 0, width: 240,
                    background: 'var(--b2)', border: '1px solid rgba(124,58,237,0.2)',
                    ...
                    <div style={{ padding: '16px 16px 14px', borderBottom: '1px solid var(--br)', display: 'flex', gap: 12, alignItems: 'center', background: 'linear-gradient(135deg, rgba(124,58,237,0.08), rgba(59,130,246,0.04))' }}>
                      <div style={{ width: 44, height: 44, borderRadius: '50%', overflow: 'hidden', border: '2px solid var(--br2)', flexShrink: 0 }}>
                        <img src={currentUser.avatarUrl || '/logo_pr.png'} alt="avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => { e.target.src = '/logo_pr.png'; }} />
                      </div>
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--t)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
